...
###############################################################################
    ...
    def df_task_states(self, task):
        ...
        # Unset prev_state for switch_in events,
        # IOW we don't care about the status of a task we are replacing
        task_switches_df.prev_state = task_switches_df.apply(
            lambda row: np.nan if row['prev_pid'] != pid else row['prev_state'], axis=1)

        task_switches_df = task_switches_df.drop(columns=["prev_pid"])

        # XXX: Sigh, here comes a nasty compatibility crud
        # To have a uniform key=value line using trace-cmd < 2.7, we have to
        # ask trace-cmd to some events unformated (-r). Sadly, this replaces
        # the state string with its integer value.
        task_switches_df.prev_state = task_switches_df.prev_state.apply(
            lambda state: TaskState(state) if isinstance(state, int) else state)

        task_switches_df.rename(columns={'prev_state' : 'curr_state'}, inplace=True)

        # We've just set curr_state (a.k.a prev_state) to NaN where task was
        # switching in, so set the state in those places to active.
        task_switches_df.curr_state = task_switches_df.curr_state.fillna(
